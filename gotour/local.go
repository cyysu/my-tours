...
func main() {
	...
	http.HandleFunc("/python", func(w http.ResponseWriter, r *http.Request) {
	...
	http.HandleFunc("/ruby", func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/ruby" {
			if err := renderRubyTour(w); err != nil {
				log.Println(err)
			}
			return
		}
